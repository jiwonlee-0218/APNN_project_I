...
class TAE(nn.Module):
    ...
    def __init__(self, args):  # 내 모델에 사용될 구성품을 정의 및 초기화하는 메서드
        ...
        self.tae_encoder = Transformer(
            vocab_size=args.input_size,
...
class Prediction_Model(nn.Module):
    ...
    def __init__(self, args, hidden_dim, num_classes):  # 내 모델에 사용될 구성품을 정의 및 초기화하는 메서드
        super(Prediction_Model, self).__init__()


        self.MLP1 = nn.Sequential(nn.Linear(hidden_dim, hidden_dim*2), nn.BatchNorm1d(hidden_dim*2), nn.ReLU(), nn.Linear(hidden_dim*2, hidden_dim*2), nn.BatchNorm1d(hidden_dim*2), nn.ReLU())
        self.MLP2 = nn.Sequential(nn.Linear(hidden_dim*2, hidden_dim), nn.BatchNorm1d(hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, num_classes))
